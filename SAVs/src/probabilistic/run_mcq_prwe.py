...
def eval_dataset(args):
    ...
    with open(summary_file, 'w') as f:
        ...
        f.write("-" * 98 + "\n")
        f.write("weight_scheme |    tau |  tau_w | entmax |  top_k | lastN | accuracy |   C |   K |   D\n")
        f.write("-" * 98 + "\n")

    # Create directories for matrices
    base_matrix_dir = os.path.join(sav_dir, f"{args.model_name}_matrices_n{args.n_trials}_{date_tag}")
    ...
    raw_reliability_matrix_dir = os.path.join(base_matrix_dir, "raw_reliability_matrix")
    counts_matrix_dir = os.path.join(base_matrix_dir, "counts_matrix")
    os.makedirs(weight_matrix_dir, exist_ok=True)
    os.makedirs(reliability_matrix_dir, exist_ok=True)
    os.makedirs(raw_reliability_matrix_dir, exist_ok=True)
    os.makedirs(counts_matrix_dir, exist_ok=True)
    
    print("=" * 80)
    print("SAVING MATRICES")
    print("=" * 80)
    print(f"Base directory: {base_matrix_dir}")
    print(f"  - Weight matrices: {weight_matrix_dir}")
    print(f"  - Reliability matrices (after shrinkage): {reliability_matrix_dir}")
    print(f"  - Raw reliability matrices (before shrinkage): {raw_reliability_matrix_dir}")
    print(f"  - Counts matrices: {counts_matrix_dir}")
    print("=" * 80)
    ...
    for lastN in lastn_list:
        ...
        for tau in tau_list:
            ...
            for ws in ws_list:
                ...
                # No shrinkage applied - use raw reliability directly
                r_hat = r
                
                # Save raw reliability matrix and counts (tau and ws dependent)
                raw_param_str_base = f"tau{tau}_ws{ws}_lastN{lastN}"
                r_np = r.cpu().numpy()  # (K, C)
                counts_np = counts.cpu().numpy()  # (C,)
                np.save(os.path.join(raw_reliability_matrix_dir, f"raw_reliability_{raw_param_str_base}.npy"), r_np)
                np.save(os.path.join(counts_matrix_dir, f"counts_{raw_param_str_base}.npy"), counts_np)
                    ...
                    for ent_alpha in entmax_alpha_list:
                        for topk in topk_list:
                            ...
                            with open(summary_file, 'a') as f:
                                f.write(f"{ws:>13} | {tau:6.3f} | {tauw:6.3f} | {ent_alpha:6.2f} | {str(topk if topk is not None else 'ALL'):>6} | {lastN:5d} | {acc:8.4f} | {C:3d} | {K:3d} | {D:3d}\n")

                            r_hat_np = r_hat.cpu().numpy()  # (K, C)
                            w_np = w.cpu().numpy()  # (K, C)
                            
                            # Create filename (no shrinkage/alpha in filename)
                            param_str = f"tau{tau}_tauw{tauw}_ent{ent_alpha}_topk{str(topk if topk is not None else 'ALL')}_ws{ws}_lastN{lastN}"
    ...
    with open(summary_file, 'a') as f:
        f.write("\nArtifacts\n")
        f.write("-" * 86 + "\n")
        f.write(f"base_matrix_dir: {base_matrix_dir}\n")
        f.write(f"weight_matrix_dir: {weight_matrix_dir}\n")
        f.write(f"reliability_matrix_dir: {reliability_matrix_dir}\n")
        f.write(f"raw_reliability_matrix_dir: {raw_reliability_matrix_dir}\n")
        f.write(f"counts_matrix_dir: {counts_matrix_dir}\n")
        f.write("\n")
        f.write("Matrix Descriptions:\n")
        f.write("  - weight_matrix: Final PRWE weights (K, C) - normalized per class\n")
        f.write("  - reliability_matrix: Reliability after shrinkage (K, C) - used to compute weights\n")
        f.write("  - raw_reliability_matrix: Raw reliability before shrinkage (K, C) - computed from validation\n")
        f.write("  - counts_matrix: Sample counts per class (C,) - used for shrinkage\n")
